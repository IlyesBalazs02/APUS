...
namespace APUS.Server.Services.Implementations.MapServices
{
	public class ActivityTrackLookupService : IActivityTrackLookupService
	{
		...
		public async Task<(double Lat, double Lon)?> FindClosestPointAsync(string activityId,string userId,DateTime photoTimeUtc,CancellationToken ct = default)
		{
			// Load activity
			...
			// Only search for lat/lon if photo time is within [start, end]
			if (photoUtc < startUtc || (endUtc.HasValue && photoUtc > endUtc.Value))
			{
				return null;
			}

			// Load trackpoints only if time window check passed
		...
		private static (double Lat, double Lon)? FindClosestBinarySearch(List<TrackpointDto> points,DateTime photoTimeUtc)
		{
			if (points.Count == 0)
				return null;
