...
namespace APUS.Server.Controllers.GroupsController
{
	...
	public sealed class GroupEventController : ControllerBase
	{
		...
		public async Task<ActionResult<GroupEventDto>> Create(
			long groupId,
			[FromBody] CreateGroupEventRequest request,
			CancellationToken ct = default)
		{
		...
		public async Task<ActionResult<IReadOnlyList<GroupEventParticipantDto>>> GetParticipants(
			long eventId,
			CancellationToken ct = default)
		{
			var userId = User.GetUserId();
